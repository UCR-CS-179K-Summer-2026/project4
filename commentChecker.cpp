...
int commentChecker(std::ifstream& inputFile){
    ...
    int functionLine = 0;//keeps track of function line for printing warning
    ...
    for (int i = 0; i < lines.size(); ++i) {//iterate through lines vector
        if (std::regex_search(lines[i], functionRegex)) {//searches for function within the line, assumes no comment until proven otherwise
            ...
            insideFunction = false;//reset these two for the loop
            braceDepth = 0;

            for(int k = i; k < lines.size(); ++k){//search function body for comments, using brace height to detect start and end
